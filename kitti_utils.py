...
from open3d.open3d import io

from kitti_config import cfg

CAM = 2   #select the second RGB camera from the KITTI Car setup


def load_cloud_and_labels(tag):
    lidar = np.fromfile(os.path.join(KITTIPaths.DIRECTORY_PATH,
                                     'velodyne', '%06d.bin' % tag), dtype=np.float32).reshape(-1, 4)
    labels = np.array([line for line in open(os.path.join(
        KITTIPaths.DIRECTORY_PATH, 'label_2', '%06d.txt' % tag), 'r').readlines()])  # (N')
...
def get_centroid(point_set):
    ...
    sumX = np.sum(point_set[:][0])
    sumY = np.sum(point_set[:][1])
    sumZ = np.sum(point_set[:][2])

    return [sumX/length, sumY/length, sumZ/length]


def translate_points(points, translation_point, direction='neg'):
    if direction == 'neg':
        for i in range(0, len(points)):
            points[i][0] -= translation_point[0]
            points[i][1] -= translation_point[1]
            points[i][2] -= translation_point[2]
    else:
        for i in range(0, len(points)):
            points[i][0] += translation_point[0]
            points[i][1] += translation_point[1]
...
def draw_lidar_box3d_on_birdview(birdview, boxes3d, scores, gt_boxes3d=np.array([]),
                                 color=(0, 255, 255), gt_color=(255, 0, 255), thickness=1, factor=1, P2=None,
                                 T_VELO_2_CAM=None, R_RECT_0=None):
    # Input:
    #   birdview: (h, w, 3)
    #   boxes3d (N, 7) [x, y, z, h, w, l, r]
    #   scores
...
def box3d_to_label(batch_box3d, batch_cls, batch_score=[], coordinate='camera', P2=None, T_VELO_2_CAM=None,
                   R_RECT_0=None):
    # Input:
    #   (N, N', 7) x y z h w l r
    #   (N, N')
        ...
        for boxes, scores, clses in zip(batch_box3d, batch_score, batch_cls):
            ...
            for box, score, cls in zip(boxes, scores, clses):
                if coordinate == 'camera':
                    ...
                    box2d = lidar_box3d_to_camera_box(
                        box[np.newaxis, :].astype(np.float32), cal_projection=False, P2=P2, T_VELO_2_CAM=T_VELO_2_CAM,
                        R_RECT_0=R_RECT_0)[0]
                x, y, z, h, w, l, r = box3d
                box3d = [h, w, l, x, y, z, r]
...
def cal_iou2d(box1, box2, T_VELO_2_CAM=None, R_RECT_0=None):
    ...
    tmp = center_to_corner_box2d(np.array([box1, box2]), coordinate='lidar', T_VELO_2_CAM=T_VELO_2_CAM,
                                 R_RECT_0=R_RECT_0)
...
# Based on the functions from voxelnet.utils, finds the best aligned bounding box, for the cropped cloud.
def get_segmented_cloud_points_after_fitting_box(box, lidar):
    # generate multiple boxes and select the best fitting one, with is_collision
    [[y, z, x, h, w, l, rot]] = box
    lidar_center_gt_box3d = camera_to_lidar_box(box)
    ...
    for idx in range(len(lidar_corner_gt_box3d)):
        ...
        while is_collision and _count < 100:
            ...
            # approximates best box position
            # check collision
            tmp = box_transform(
        ...
        if not is_collision:
            ...
            bbox_center = np.array([(np.sum(box_corner[:, 0], dtype=np.float64) / len(box_corner[:, 0])),
                                    (np.sum(box_corner[:, 1], dtype=np.float64) / len(box_corner[:, 1])),
                                    (np.sum(box_corner[:, 2], dtype=np.float64) / len(box_corner[:, 2]))])
            ...
            # draw_bounding_box_on_cloud(minx, miny, minz,maxx, maxy, maxz, lidar)
            ...
            segmented_cloud_points = lidar[bound_box, 0:3]

    return segmented_cloud_points, bbox_center
